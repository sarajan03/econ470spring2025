...
question2 <-ggplot(avg_benchmark, aes(x = year, y = average_benchmark)) +
...
```{r}
# Estimation for the effect of receiving a 3-star versus a 2.5-star rating
rd_effect_2_5_3 <- lm(mkt_share ~ treatment_indicator + score_diff, 
                      data = (raw_2010 %>% 
                              filter(raw_rating >= (2.75 - 0.125), 
                                     raw_rating <= (2.75 + 0.125), 
                                     Star_Rating %in% c(2.5, 3.0)) %>%
                              mutate(treatment_indicator = (Star_Rating == 3.0), 
                                     score_diff = raw_rating - 2.75)))

summary(rd_effect_2_5_3)

# Estimation for the effect of receiving a 3.5-star versus a 3-star rating
rd_effect_3_3_5 <- lm(mkt_share ~ treatment_indicator + score_diff, 
                      data = (raw_2010 %>% 
                              filter(raw_rating >= (3.25 - 0.125), 
                                     raw_rating <= (3.25 + 0.125), 
                                     Star_Rating %in% c(3.0, 3.5)) %>%
                              mutate(treatment_indicator = (Star_Rating == 3.5), 
                                     score_diff = raw_rating - 3.25)))

summary(rd_effect_3_3_5)

# Creating a table to summarize the results
# create a table from both 
### extract tidy results
tidy_30 <- tidy(rd_effect_2_5_3)
tidy_35 <- tidy(rd_effect_3_3_5 )
